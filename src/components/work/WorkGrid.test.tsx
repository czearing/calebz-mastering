...
describe("WorkGrid", () => {
  it("renders one card button per track", () => {
    ...
    expect(screen.getByRole("button", { name: /Alpha by One/i })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /Beta by Two/i })).toBeInTheDocument();
  });
  ...
  it("opens the modal for the clicked track only", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /Beta by Two/i }));
  ...
  it("returns focus to the trigger card when the modal closes", async () => {
    const user = userEvent.setup();
    render(<WorkGrid tracks={tracks} />);
    const trigger = screen.getByRole("button", { name: /Alpha by One/i });
